...
module movemate::table {
    use sui::object::{Self, ID};
    use sui::tx_context::{Self, TxContext};
    friend movemate::table_with_length;
    ...
    public fun add<K: copy + drop, V>(ctx: &mut TxContext, table: &mut Table<K, V>, key: K, val: V) {
        let uid = object::new(ctx);
        add_box<K, V, Box<V>>(table, key, Box{id: object::uid_to_inner(&uid), val: val});
        object::delete(uid);
    }
    ...
    public fun borrow_mut_with_default<K: copy + drop, V: drop>(ctx: &mut TxContext, table: &mut Table<K, V>, key: K, default: V): &mut V {
        if (!contains(table, copy key)) {
            add(ctx, table, copy key, default)
        };
    ...
    public fun upsert<K: copy + drop, V: drop>(ctx: &mut TxContext, table: &mut Table<K, V>, key: K, value: V) {
        if (!contains(table, copy key)) {
            add(ctx, table, copy key, value)
        } else {
    ...
    public fun remove<K: copy + drop, V>(table: &mut Table<K, V>, key: K): V {
        let Box{ id, val } = remove_box<K, V, Box<V>>(table, key);
        val
    }
