...
module movemate::table_with_length {
    use movemate::table::{Self, Table};
    ...
    public fun destroy_empty<K: copy + drop, V>(table: TableWithLength <K, V>) {
        assert!(table.length == 0, ENOT_EMPTY);
    ...
    public fun add<K: copy + drop, V>(ctx: &mut TxContext, table: &mut TableWithLength <K, V>, key: K, val: V) {
        table::add(ctx, &mut table.inner, key, val);
    ...
    public fun borrow_mut_with_default<K: copy + drop, V: drop>(ctx: &mut TxContext, table: &mut TableWithLength <K, V>, key: K, default: V): &mut V {
        if (table::contains(&table.inner, key)) {
            table::borrow_mut(&mut table.inner, key)
        } else {
            table::add(ctx, &mut table.inner, key, default);
            table.length = table.length + 1;
            table::borrow_mut(&mut table.inner, key)
        }
    }
