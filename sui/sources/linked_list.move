...
/// ```
module movemate::linked_list {
    use movemate::table_with_length as table;
    ...
    public fun singleton<V: store + copy + drop>(ctx: &mut TxContext, val: V): LinkedList<V> {
        let list = new();
        add(ctx, &mut list, val);
        list
    }

    /// Add a value to the list.
    public fun add<V: store + copy + drop>(ctx: &mut TxContext, list: &mut LinkedList<V>, value: V) {
        let key = list.keyCounter;
        list.keyCounter = list.keyCounter + 1;

        let nodeKeys = table::borrow_mut_with_default(ctx, &mut list.nodeKeys, value, vector::empty());
        ...
        table::add(ctx ,&mut list.nodes, key, node);
